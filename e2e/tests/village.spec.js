...
test('the Chromebook-sized 3D village lets a student walk to the Elder and press E', async ({ page }) => {
  ...
  for (let step = 0; step < 5; step += 1) {
    ...
    nearElder = await playerPosition(page);
    if (nearElder.z < 22.3) break;
  }
  expect(nearElder.z).toBeLessThan(22.3);
  ...
  await expect(page.getByRole('button', { name: 'Accept Chapter 1' })).toBeFocused();
  ...
  await expect(page.locator('#dialogue-box')).toBeHidden();
  await expect(page.getByRole('button', { name: 'Visit the Village Elder' })).toBeFocused();
  await page.keyboard.press('e');
  await expect(page.locator('#dialogue-box')).toBeVisible();
  ...
  const resumedPosition = await playerPosition(page);
  expect(resumedPosition.z).toBeGreaterThan(nearElder.z);
  await page.keyboard.press('e');
  await expect(page.locator('#dialogue-box')).toBeVisible();
