...
After choosing a Guardian and realm, move the Guardian through the 3D village with **WASD** or the **arrow keys**. Walk near the Village Elder at the gate and press **E** to accept Chapter 1. Follow the path past Kokura Keep to the broken beacon and press **E** there to begin planning and the writing battle. Claiming victory lights the beacon; the light stays on after **Continue Journey**. The Market Garden, Harbour Chapel and Kokura Keep can also be explored with **E**. The camera follows the Guardian, and the Guardian stays on the island paths. The village position is saved on this device.

The **Visit the Village Elder** button remains available for keyboard and assistive technology users. When WebGL is unavailable, an illustrated village and **Begin at the Broken Beacon** button keep Chapter 1 playable. The game uses one lightweight procedural scene and does not require an AI service to move or play.
...
The browser suite walks to the Elder and beacon, checks movement and interaction, then completes Chapter 1, lights the beacon, claims and equips the reward, saves the game, refreshes the page and verifies **Continue Journey** restores progress. It also checks the Challenge-mode controls and the WebGL fallback.
